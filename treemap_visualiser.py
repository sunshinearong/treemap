...
def run_visualisation(tree):
    ...
    render_display(screen, tree, 'leaf name')
...
def render_display(screen, tree, text):
    ...
    pygame.draw.rect(screen, pygame.color.THECOLORS['black'],
                     (0, 0, WIDTH, HEIGHT))
    rects = tree.generate_treemap((0, 0, WIDTH, HEIGHT))#get place,size and color
    for rectangle in rects:
        pygame.draw.rect(screen, rectangle[1],rectangle[0])#draw leaf according to rects
    _render_text(screen, text)#show text for selected leaf
...
def event_loop(screen, tree):
    ...
    while True:
        # Wait for an event
        selected_leaf = str('')#record selected rectangle name 
        event = pygame.event.poll()
        if event.type == pygame.QUIT:
            return
        if event.type == pygame.KEYUP:#press any key to quit
            return
        if event.type == pygame.MOUSEBUTTONUP:#click mouse
            selected_leaf = tree.get_separator(event.pos,(0, 0, WIDTH, HEIGHT))#get name of selected leaf
            changed_tree = tree.change_leaf_size(selected_leaf,event.button)#change size for specify leaf and its parent
            render_display(screen, changed_tree, selected_leaf) #update screen
