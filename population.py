...
def _load_data():
    ...
    region_trees = []#list of regions trees
    for region,countries in regions.iteritems():
        subtrees = [PopulationTree(world=False,root = country,data_size = country_populations[country]) \
                    for country in countries if country in country_populations.keys()]   #create subtree for every country in current region 
        region_trees.append(PopulationTree(world=False,root = country,subtrees = subtrees))#add subtrees created to regions_tree
    return region_trees
...
def _get_population_data():
    ...
    for population in population_data:
        if population['value'] != None:#population is not zero
            country = population['country']['value']
            number = int(str(population['value']))
...
def _get_region_data():
    ...
    for data in country_data:
        key = data['region']['value']
        value = data['name']
        if  key in regions.keys():#key already exists,just append value
            regions[key].append(value)
        else:
            regions[key] = [value]#key does not exist,add key to dict
